...
import type { RouteWithSchedules } from "../hooks/useRouteSchedules";

interface DeleteConfirmationModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: () => Promise<void>;
  route: RouteWithSchedules | null;
  isDeleting: boolean;
}
      ...
      <DialogContent className="bg-card text-card-foreground border-border">
        <DialogHeader>
          ...
          <DialogDescription className="text-muted-foreground">
            ¿Estás seguro que deseas eliminar todos los horarios de la ruta <span className="font-bold text-foreground">{route.route.numberRoute}</span>?
          </DialogDescription>
        </DialogHeader>
